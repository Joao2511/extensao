...
/** Por linha, os intervalos [início, fim) de caracteres escondidos. */
export type Hidden = Array<Array<[number, number]>>;

/** Palavras que dão a estrutura da linha; ficam sempre visíveis. */
const KEYWORDS = new Set(
  (
    'if else elif for while do return function def class new this self import export from as async await try catch except finally ' +
    ...
    'in of not and or is with lambda yield throw raise super extends implements interface type enum package namespace using'
  ).split(' '),
);
...
function candidates(line: string, limit: number): [number, number][] {
  ...
  while ((m = token.exec(text))) {
    ...
    if (quote === '"' || quote === "'" || quote === '`') {
      if (found.length > 2) out.push([start + 1, start + found.length - 1]); // aspas ficam como dica
      continue;
    }
    if (found.length < 2 || KEYWORDS.has(found)) continue;
 ...
 */
export function chooseHidden(lines: string[], required: number[] | undefined, ratio: number): Hidden {
...
export function mask(line: string, ranges: [number, number][] | undefined, fill = '_'): string {
  if (!ranges?.length) return line;
  let out = line;
  for (const [a, b] of ranges) out = out.slice(0, a) + fill.repeat(b - a) + out.slice(b);
  return out;
}
