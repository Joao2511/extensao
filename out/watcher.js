...
class FileWatcher {
    ...
    snapshot = new Map();
    savedAt = new Map();
    pending = new Map(); // caminho -> foi criado agora
    timer;
    constructor(ctx, receiver, out) {
        ...
        ctx.subscriptions.push(vscode.workspace.onDidSaveTextDocument((d) => this.savedAt.set(this.key(d.uri.fsPath), Date.now())), 
        // Arquivos além do limite inicial entram na cópia quando são abertos.
        vscode.workspace.onDidOpenTextDocument((d) => {
            if (d.uri.scheme === 'file' && !this.snapshot.has(this.key(d.uri.fsPath)))
                void this.remember(d.uri.fsPath);
        }), { dispose: () => this.timer && clearTimeout(this.timer) });
        void this.buildSnapshot();
    }
    get on() {
        return vscode.workspace.getConfiguration('aprender').get('observarArquivos', true);
    }
    log(msg) {
    ...
    ignored(p) {
        if (this.key(p).startsWith(this.key(HOME_APRENDER)))
            return true;
        ...
        if (parts.some((seg) => SKIP_DIRS.has(seg)))
            return true;
    ...
    async remember(p) {
        ...
        if (text !== undefined)
            this.snapshot.set(this.key(p), text);
    }
    async buildSnapshot() {
        const t0 = Date.now();
    ...
    async flush() {
        ...
        for (const [p, created] of batch)
            await this.handle(p, created);
    }
    async handle(p, created) {
        const k = this.key(p);
        ...
        if (Date.now() - (this.savedAt.get(k) ?? 0) < SAVE_GRACE_MS)
            return; // foi o VS Code salvando
        if (!this.on)
            return;
        if (before === now)
            return;
    ...
    async synced(uri, disk) {
        const norm = (t) => t.replace(/\r\n/g, '\n');
        const target = norm(disk);
        const doc = await vscode.workspace.openTextDocument(uri);
        if (norm(doc.getText()) === target)
            return doc;
        if (doc.isDirty)
            return undefined;
        return new Promise((resolve) => {
            ...
            const sub = vscode.workspace.onDidChangeTextDocument((e) => {
                if (e.document === doc && norm(doc.getText()) === target)
                    done(doc);
            });
...
function changedBlock(before, after) {
    ...
    while (start <= end && b[start].trim() === '')
        start++;
    while (end >= start && b[end].trim() === '')
        end--;
