...
class Explainer {
    ...
    async explain(req) {
        ...
        if (envelope.is_error)
            throw new Error(String(envelope.result || 'erro desconhecido do claude'));
        const parts = this.normalize(envelope.structured_output?.parts, req.lines.length);
        if (!parts.length)
            throw new Error('a IA não devolveu partes');
        try {
    ...
    run(req) {
        ...
        const env = { ...process.env, APRENDER_SKIP_HOOK: '1' };
