...
class Walkthrough {
    mgr;
    out;
    controller = vscode.comments.createCommentController('aprender', 'Aprender');
    tours = new Map();
    explainer;
    ...
    constructor(ctx, mgr, out) {
        this.mgr = mgr;
        this.out = out;
        ctx.subscriptions.push(this.controller, this.highlight, this.status, vscode.window.onDidChangeTextEditorSelection((e) => this.followCursor(e.textEditor)), vscode.window.onDidChangeActiveTextEditor((ed) => ed && this.paint(ed)), vscode.workspace.onDidCloseTextDocument((doc) => this.clear(doc.uri.toString())));
    }
    get config() {
    ...
    async start(editor, startLine, lines) {
        ...
        const explainer = new explainer_1.Explainer(this.findBinary());
        this.explainer = explainer;
        const model = this.config.get('explicar.modelo', 'sonnet').trim() || 'sonnet';
        const effortCfg = this.config.get('explicar.effort', 'padrão').trim();
        ...
        if (this.explainer !== explainer)
            return; // outro pedido começou enquanto este esperava
    ...
    async chooseModel() {
        const current = this.config.get('explicar.modelo', 'sonnet');
        const items = [
    ...
    async chooseEffort() {
        const current = this.config.get('explicar.effort', 'padrão');
        const items = [
    ...
    finish() {
        const editor = vscode.window.activeTextEditor;
        if (editor)
            this.clear(editor.document.uri.toString());
    }
    step(delta) {
        const editor = vscode.window.activeTextEditor;
    ...
    show(editor, index, reveal) {
        const tour = this.tours.get(editor.document.uri.toString());
        if (!tour || index < 0 || index >= tour.parts.length)
            return;
        tour.current = index;
        ...
        this.paint(editor);
        if (reveal) {
            editor.revealRange(this.rangeOf(editor.document, tour.startLine, tour.parts[index]), vscode.TextEditorRevealType.InCenterIfOutsideViewport);
        }
    }
    paint(editor) {
        const tour = this.tours.get(editor.document.uri.toString());
    ...
    followCursor(editor) {
        ...
        if (idx >= 0 && idx !== tour.current)
            this.show(editor, idx, false);
    }
    clear(key) {
        const tour = this.tours.get(key);
        if (!tour)
            return;
        for (const t of tour.threads)
            t.dispose();
